...
class Worker
{
    ...
    protected static function init()
    {
        ...
        {
            self::$_startFile = TP_COMMAND_FILE;
        }else{
    ...
    {
        //start =------------------ 兼容TP的Command模式----------------
        if (defined("TP_COMMAND")) {
            $tempArgv = $GLOBALS["argv"];
            $argv[0] = $tempArgv[0];
            unset($tempArgv[0]);
            $commnadStr = "|start|stop|restart|reload|status|kill|";
            foreach ($tempArgv as $key => $arg) {
                if (strpos($commnadStr, "|$arg|") !== false) {
                    $argv[] = $arg;
                }
            }
        } else {
...
//        global $argv;
    ...
    protected static function saveMasterPid()
    {
        ...
        print_r(file_get_contents(self::$pidFile, self::$_masterPid));
    }
